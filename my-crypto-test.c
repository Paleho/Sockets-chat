...
static int test_crypto(int cfd)
{
	 ...
	 */
    memcpy(data.in, "hello\0", sizeof("hello\0"));
	// if (fill_urandom_buf(data.in, DATA_SIZE) < 0) {
	// 	perror("getting data from /dev/urandom\n");
	// 	return 1;
	// }

    memcpy(data.key, "ae9sPk4FaXadnaF0", KEY_SIZE);
	memcpy(data.iv, "8hAM7sD6RMUrjp80", KEY_SIZE);

    printf("Testing char c = %c, x = %x\n", '\0', '\0');
	printf("\nOriginal data (size = %ld):\n", sizeof(data.in));
